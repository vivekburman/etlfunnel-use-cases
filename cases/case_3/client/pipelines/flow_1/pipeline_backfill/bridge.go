package client_pipeline_backfill

// Historical Backfill Pipeline — flow_1
//
// Fetches 730 days of GA4 session data (one calendar-day per replica) from
// analyticsdata.googleapis.com (or the local seeder) and upserts into
// dbo.ga4_sessions via a staging-then-MERGE pattern.
//
// Transformer chain (applied in order):
//   transformer_1  DateParser
//   transformer_2  DimensionNormaliser
//   transformer_3  SurfaceInjector
//   transformer_4  PropertyInjector
//   transformer_5  NullFiller
//   transformer_6  MetricTypeCaster
//   transformer_7  QuotaThrottle
...
func PerformOperations(
	...
	_logger.Debug("backfill pipeline starting")
	...
	_logger.Debug("backfill pipeline complete")
}
...
func processRecords(
	...
	for {
		...
		case <-pipeline.pcm.GetContext().Done():
			pipeline.log.Debug("context cancelled")
			return
...
func applyTransformations(record map[string]any, pipeline *pipelineContext) (map[string]any, error) {
	pipeline.transformerParam.Record = record
	var (
