...
// These are canonical post-schema-mapper names (transformer_26 renames source
// columns before this transformer runs).
var dateColumns = map[string]bool{
	"dob":              true, // customers
	"activation_date":  true, // customers
	"plan_start":       true, // subscriptions
	"plan_end":         true, // subscriptions
	"cycle_start":      true, // billing_accounts
	"cycle_end":        true, // billing_accounts
	"activated_date":   true, // sim_inventory
	"deactivated_date": true, // sim_inventory
	"port_date":        true, // port_history
}

// decimalColumns are DECIMAL fields stored as []byte by the MySQL driver.
// Uses canonical post-schema-mapper names.
var decimalColumns = map[string]bool{
	"amount_due":  true, // billing_accounts
	"amount_paid": true, // billing_accounts
}
