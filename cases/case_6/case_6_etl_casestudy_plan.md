...
2. **One fixed `Directory` per connector, but Flow 36's data rotates to a new sub-directory every day.** `DBParquetConfig`/`DBAvroConfig` each carry a single `Directory` string, resolved once at connect time — it cannot be re-pointed at a new `dt=<date>/` sub-folder on its own each day. So Flow 36's `Directory` (in `collection.json`'s `source.connectionParams`) is fixed at the **stable base path** (`FC_SNAPSHOT_DIR`), and `GenerateScan` builds each `Files` entry as `dt=<date>/part-00000.parquet` — relative to that stable base, with the day's sub-folder baked into the entry itself — rather than assuming `Directory` already points at the dated folder. Flow 37's `Directory` has no such problem: WMS never rotates its export folder, so `Directory` and the AuxDB checkpoint key are the same fixed path for the life of the pipeline, and `Files` entries are bare filenames.
3. **No watch/poll mode in the framework, but also no live connection to keep open.** `ReadByFullScan` reads exactly the `Files` list `GenerateScan` resolved at the start of a run, then closes its own channel once every file/row has been delivered — there's no filesystem watcher, but also nothing analogous to a REST/Kafka connector's persistent connection to hold open while waiting for more data. Both flows terminate the same way: the channel closes on its own, and `PerformOperations` treats "channel closed, no failure, no terminate-rule stop" as a normal finish. The real difference between the flows is **invocation cadence, not termination logic** — Flow 36 is invoked once per day against a fresh directory and reaches a genuine "this directory is done" state (`fc_snapshot_progress.status='complete'`), while Flow 37 is meant to be re-invoked periodically (cron/manual trigger) against the same growing directory, and every invocation just processes whatever new part files exist at that moment before finishing the same way. `MaxPipelineTime` on both flows' terminate rules is a safety net for a wedged run, not the mechanism that ends a normal one.
...
The transformer chain only ever sees `Record.Data` — `Meta` never flows through a transformer and never lands in a MongoDB document by default. The checkpoint writer reads `record.Meta[models.MetaFilePart]`/`record.Meta[models.MetaFileRow]` directly to persist `(part, row)` after every successful destination write, exactly the way Case 4 persists Kafka offsets — just addressing a position in a static, client-enumerated file list rather than a live partition/offset pair or a REST cursor.
...
|---|---|---|---|
| Flow 36 (Parquet) | **A** | `sku_id = ""` | `transformer_97` returns `nil` — silently dropped, `records-dropped` metric incremented (an empty SKU can't form the MongoDB upsert key) |
| Flow 36 (Parquet) | **B** | `quantity_on_hand = -1` | `transformer_98` returns an error — routed to `fc_snapshot_backlog` |
| Flow 37 (Avro) | **A** | `fc_id = ""` | `transformer_100` returns `nil` — silently dropped (a movement can't be attributed to any FC) |
| Flow 37 (Avro) | **B** | `movement_type = "UNKNOWN"` | `transformer_101` returns an error — routed to `fc_movement_backlog` |
...
|---|---|---|---|
| `fc_inventory` | `inventory_snapshots` | Flow 36 | connector_63 / iso_entity_153 |
| `fc_inventory` | `stock_movements` | Flow 37 | connector_65 / iso_entity_155 |
...
`transformer_99` (`MongoKeyStamper`) stamps `_mongo_id = "fc_id|sku_id|snapshot_date"` onto the record's `Data`, not `_id` directly — `_mongo_id` is what connector_63's `GenerateQuery` reads to build the `{_id: mongoID}` upsert filter, and MongoDB assigns that filter value as the new document's `_id` on insert. Neither `_mongo_id` nor `_file_part`/`_file_row` (re-attached from `Record.Meta` onto `Record.Data` by flow_36's bridge, purely for the checkpoint writer to see — `Record.Meta` never reaches the transformer chain or `CheckpointProps`/`BacklogProps` on its own) appear in the document: connector_63 strips every underscore-prefixed key from the payload before writing, the same convention Cases 4–5 use for `_kafka_*`/`_es_*` routing keys.
...
`transformer_102` stamps `_mongo_id = movement_id` (already globally unique, so no composite key needed). Unique index: `{ movement_id: 1 }`. As with `inventory_snapshots`, `_mongo_id`/`_file_part`/`_file_row` stay off the document — stripped by connector_65's `GenerateQuery`.
...
```go
// connector_63/iso_entity_153 (inventory_snapshots) and connector_65/iso_entity_155
// (stock_movements) both read "_mongo_id" — transformer_99/transformer_102 stamp it
...
### 3.1 `fc_snapshot_progress` — Flow 36 checkpoints, keyed by directory
...
### 3.2 `fc_snapshot_backlog` — Flow 36 failed rows
...
### 3.3 `fc_movement_progress` — Flow 37 checkpoint, keyed by directory (never changes)
...
### 3.4 `fc_movement_backlog` — Flow 37 failed records
...
### 4.1 Flow 36 — Parquet Snapshot Ingestion Flow

**Purpose**: Full-scan a day's Parquet snapshot directory, validate and upsert every row into MongoDB.

**Source connector**: `IParquetNoSQLSource` (connector_62 / iso_entity_152) — `ReadByFullScan`
**Destination**: MongoDB `fc_inventory.inventory_snapshots` (connector_63 / iso_entity_153)
**Checkpoint**: `fc_snapshot_progress`, keyed by directory (checkpoint_15)
**Pipeline orchestrator**: orchestrator_21

**Execution model**:
1. orchestrator_21 resolves `directory` (`<FC_SNAPSHOT_DIR>/dt=<date>`) and `dateSubdir` (`dt=<date>`) and stashes both in `ReplicaProps`.
...
4. flow_36's bridge copies each record's `Meta[MetaFilePart]`/`Meta[MetaFileRow]` onto `Data["_file_part"]`/`Data["_file_row"]`, then runs the transformer chain.
5. Upsert passing rows into `inventory_snapshots`.
6. After each successfully written batch, upsert `fc_snapshot_progress` with the highest `(_file_part, _file_row)` seen in that batch.
7. When the source channel closes on its own (every listed file/row delivered) with no failure and no terminate-rule stop, `checkpoint_15.MarkComplete` sets `status='complete'` for that directory — this is a plain "the scan finished normally" signal, not a distinct rule the terminate check polls for.
8. On restart before completion: `GenerateScan` re-reads `(last_part, last_row)` and resumes from step 3.

**Termination**: `terminate_15` registers only a `MaxPipelineTime` safety net (30 min) — a normal run never hits it; it just finishes when its bounded `Files` list is exhausted and the channel closes.

**Flow 36 transformer chain**:

| Transformer | ID | Responsibility |
|---|---|---|
| `SKUValidator` | transformer_97 | `sku_id == ""` → `nil` (silent drop, Fault A). Otherwise passthrough. |
| `QuantityGuard` | transformer_98 | `quantity_on_hand < 0` → error (Fault B, routes to `fc_snapshot_backlog`). Otherwise passthrough. |
| `MongoKeyStamper` | transformer_99 | Stamps `_mongo_id = fc_id + "|" + sku_id + "|" + snapshot_date` for idempotent upsert. |

### 4.2 Flow 37 — Avro Movement Ingestion Flow

**Purpose**: Full-scan the (ever-growing) Avro movement directory, validate and upsert every record into MongoDB.

**Source connector**: `IAvroNoSQLSource` (connector_64 / iso_entity_154) — `ReadByFullScan`
**Destination**: MongoDB `fc_inventory.stock_movements` (connector_65 / iso_entity_155)
**Checkpoint**: `fc_movement_progress` (single row, directory never changes; checkpoint_16)
**Pipeline orchestrator**: orchestrator_22

**Execution model**:
1. orchestrator_22 resolves the fixed `directory` (`FC_MOVEMENT_DIR`) into `ReplicaProps` — no date logic.
...
4. flow_37's bridge copies `Meta[MetaFilePart]`/`Meta[MetaFileRow]` onto `Data["_file_part"]`/`Data["_file_row"]`, then runs the transformer chain.
5. Upsert passing records into `stock_movements`.
6. After each successfully written batch, upsert `fc_movement_progress` with the highest `(_file_part, _file_row)` seen.
...
**Termination**: `terminate_16` registers only a `MaxPipelineTime` safety net (15 min), same reasoning as Flow 36 — a normal run finishes when the channel closes, not via an active idle/exhaustion check.

**Flow 37 transformer chain**:

| Transformer | ID | Responsibility |
|---|---|---|
| `FCIDValidator` | transformer_100 | `fc_id == ""` → `nil` (silent drop, Fault A). Otherwise passthrough. |
| `MovementTypeValidator` | transformer_101 | `movement_type` not one of `PUTAWAY/PICK/RETURN/DAMAGE/ADJUSTMENT` → error (Fault B, routes to `fc_movement_backlog`). Otherwise passthrough. |
| `MongoKeyStamper` | transformer_102 | Stamps `_mongo_id = movement_id` (already globally unique — no composite key needed). |

---

## Part 5 — Connector Implementation Mapping

### 5.1 `IClientDBParquetSource` — Flow 36 (connector_62 / iso_entity_152)
...
| Method | Flow 36 Usage |
|---|---|
| `GenerateScan` | Reads `directory`/`dateSubdir` from `param.State.GetReplicaProps()` (stamped by orchestrator_21); reads `(last_part, last_row)` from `fc_snapshot_progress` for that `directory`; calls `helper.ListParts(directory, ".parquet")` to enumerate the dated sub-directory itself (the connector won't); builds each `Files` entry as `filepath.Join(dateSubdir, filepath.Base(p))` so it resolves correctly against the connector's *stable* configured `Directory`; returns `&models.ParquetSourceScanTune{Files: files, RowLimit: 0, StartAfterPart: last_part, StartAfterRow: last_row}` |
| `FetchRecords` | Not used — Flow 36 runs exclusively in `ReadByFullScan` mode (`captureMethod=1`). Stubbed with a closed empty channel. |

### 5.2 `IClientDBAvroSource` — Flow 37 (connector_64 / iso_entity_154)
...
|---|---|---|---|
| connector_63 / iso_entity_153 | `inventory_snapshots` | Flow 36 | `_mongo_id` (stamped by transformer_99) |
| connector_65 / iso_entity_155 | `stock_movements` | Flow 37 | `_mongo_id` (stamped by transformer_102) |
...
Collection  = FC Inventory Ops Pipeline
  │
  ├── Flow 36 — Parquet Snapshot Ingestion (pid=36)   [Parquet dir → MongoDB]
  │     pipeline orchestrator: orchestrator_21 (PID 21)
  │     └── Pipeline 1
  │           source:      connector_62 / iso_entity_152 — Parquet directory (dt=<date>) full scan
  │           transformers: transformer_97 → transformer_98 → transformer_99
  │           destination: connector_63 / iso_entity_153 — MongoDB fc_inventory.inventory_snapshots
  │           checkpoint:  checkpoint_15 → fc_snapshot_progress (AuxDB, keyed by directory)
  │           backlog:     backlog_15 → fc_snapshot_backlog (AuxDB)
  │           terminate:   terminate_15 (MaxPipelineTime safety net only)
  │           destinationWrite: destinationwrite_13
  │
  └── Flow 37 — Avro Movement Ingestion (pid=37)      [Avro dir → MongoDB]
        pipeline orchestrator: orchestrator_22 (PID 22)
        └── Pipeline 1
              source:      connector_64 / iso_entity_154 — Avro directory full scan (flat, never rotates)
              transformers: transformer_100 → transformer_101 → transformer_102
              destination: connector_65 / iso_entity_155 — MongoDB fc_inventory.stock_movements
              checkpoint:  checkpoint_16 → fc_movement_progress (AuxDB, single row)
              backlog:     backlog_16 → fc_movement_backlog (AuxDB)
              terminate:   terminate_16 (MaxPipelineTime safety net only)
              destinationWrite: destinationwrite_14
```
...
|---|---|---|---|
| connector_62 / iso_entity_152 | Parquet directory | `IParquetNoSQLSource` / `IClientDBParquetSource` | Flow 36 source |
| connector_63 / iso_entity_153 | MongoDB `fc_inventory.inventory_snapshots` | `IMongoDBDestination` / `IClientDBMongoDest` | Flow 36 destination |
| connector_64 / iso_entity_154 | Avro directory | `IAvroNoSQLSource` / `IClientDBAvroSource` | Flow 37 source |
| connector_65 / iso_entity_155 | MongoDB `fc_inventory.stock_movements` | `IMongoDBDestination` / `IClientDBMongoDest` | Flow 37 destination |

### 6.3 Concurrency Model

```
Flow 36:  1 sequential part-file reader × 1 Mongo writer = 1 active goroutine lane
Flow 37:  1 sequential part-file reader × 1 Mongo writer = 1 active goroutine lane
...
4. Running the snapshot seeder again for a **new** `SNAPSHOT_DATE` creates a fresh directory — Flow 36 sees it as an entirely new, independent scan.
...
4. Appends new `part-000NN.avro` files starting from that sequence number — **existing part files are never touched**, so re-running the movement seeder simulates "WMS exported another batch," and Flow 37's checkpoint naturally picks up only the new parts on its next invocation.
...
|---|---|---|
| **Flow 36 snapshot progress** | `fc_snapshot_progress` | `directory`, `last_part`, `last_row`, `status`, `updated_at` — one row per day processed |
| **Flow 37 movement progress** | `fc_movement_progress` | `last_part`, `last_row`, `updated_at` — confirms Flow 37 is still advancing across the growing directory |
...
── Flow 36: Parquet → MongoDB (snapshot progress) ────────────────────────────
  directory=dt=2026-07-10   part=1   row=412   status=in_progress   updated=09:41:10

── Flow 37: Avro → MongoDB (movement progress) ───────────────────────────────
...
── Backlogs ──────────────────────────────────────────────────────────────────
  fc_snapshot_backlog (Flow 36 failed rows)    : 4
  fc_movement_backlog (Flow 37 failed records) : 6
...
|---|---|---|
| Snapshot progress stuck mid-directory | 3+ consecutive ticks | Flow 36 stalled — MongoDB unreachable, or a bad row wedged in a fixed-size batch |
| Movement progress not advancing | No change since the last `make seed-movement` | Normal — Flow 37 already finished the parts that existed at its last invocation; investigate only if new part files exist on disk and a re-run still doesn't advance the checkpoint |
| Snapshot backlog growing | Rate > 5% of rows | Fault rate high, or `transformer_98` (QuantityGuard) threshold misconfigured |
| Movement backlog growing | Rate > 5% of records | Unexpected `movement_type` values — check whether WMS introduced a new enum member upstream (`transformer_101`) |
...
**Pipeline environment variables** (set before running streamcraftexecution; also read directly by orchestrator_21/22):
```bash
FC_SNAPSHOT_DIR=/data/fc_inventory_snapshots   # Flow 36 orchestrator resolves <this>/dt=<date>/ per run
SNAPSHOT_DATE=2026-07-10                       # optional override; defaults to today (UTC)
FC_MOVEMENT_DIR=/data/wms_stock_movements      # Flow 37 — fixed, never changes
...
- [ ] **STEP-12** — Scaffold `client/connectors/connector_62/iso_entity_152` implementing `IClientDBParquetSource` as a stateless `IUseConnector{}`, exposed via a package-level `Instance` (matching the framework's convention for source bridges even though nothing here needs cross-call state).
- [ ] **STEP-13** — Implement `GenerateScan` — reads `directory`/`dateSubdir` from `param.State.GetReplicaProps()` (stamped by orchestrator_21); calls `helper.ListParts(directory, ".parquet")` to enumerate the dated sub-directory's `part-*.parquet` files ascending (the connector itself never does this); builds each `Files` entry as `filepath.Join(dateSubdir, filepath.Base(p))` so it resolves against the connector's *stable* configured `Directory`; queries `fc_snapshot_progress` for the row matching `directory` (via `ulib.GetAuxPostgresConn`), if absent treats as `(0, 0)`. Returns `&models.ParquetSourceScanTune{Files: files, RowLimit: 0, StartAfterPart: lastPart, StartAfterRow: lastRow}`.
...
- [ ] **STEP-15** — Scaffold `client/connectors/connector_64/iso_entity_154` implementing `IClientDBAvroSource`, same stateless `IUseConnector{}` + `Instance` shape as STEP-12.
- [ ] **STEP-16** — Implement `GenerateScan` — reads the fixed `directory` from `ReplicaProps` (stamped by orchestrator_22); calls `helper.ListParts(directory, ".avro")` fresh every invocation, so parts appended by a later `movement_seeder` run are included automatically; returns bare filenames (no sub-folder rewrite needed, unlike STEP-13); queries the single `fc_movement_progress` row, if absent treats as `(0, 0)`. Returns `&models.AvroSourceScanTune{Files: files, RowLimit: 0, StartAfterPart: lastPart, StartAfterRow: lastRow}`.
- [ ] **STEP-17** — Implement `FetchRecords` with the same reasoning as STEP-14.

### Phase 5 — MongoDB Destination Connectors

- [ ] **STEP-18** — Scaffold **two** connector packages — `client/connectors/connector_63/iso_entity_153` and `client/connectors/connector_65/iso_entity_155` — both implementing `IClientDBMongoDest` as a stateless `IUseConnector{}`. Two packages, not one, because `DBMongoConfig.Collection` is fixed at connect time; there is no branch-on-caller design that would let a single connector serve both collections.
- [ ] **STEP-19** — Implement `GenerateQuery` for connector_63 (`inventory_snapshots`): builds one `MongoDestQueryTune` per record with `Query = bson.M{"_id": rec["_mongo_id"]}`, `Payload = bson.M{"$set": doc}` (`doc` = `rec` with every `_`-prefixed key stripped), `Operation: models.MongoWriteUpdateOne`, `Options: models.MongoDBWriteOptions{Upsert: true}`. Errors if `_mongo_id` is missing.
- [ ] **STEP-20** — Implement `GenerateQuery` for connector_65 (`stock_movements`): identical shape to STEP-19 — both connectors read the same `_mongo_id` key; only the stamped value's shape differs (composite key vs. `movement_id` directly), and that's decided by the transformer, not the connector.

### Phase 6 — Transformer Chains

- [ ] **STEP-21** — Implement `transformer_97` (`SKUValidator`) for Flow 36: `sku_id == ""` → `nil` (silent drop). Otherwise passthrough.
- [ ] **STEP-22** — Implement `transformer_98` (`QuantityGuard`) for Flow 36: `quantity_on_hand < 0` → error `"transformer_98: negative quantity_on_hand"`. Otherwise passthrough.
- [ ] **STEP-23** — Implement `transformer_99` (`MongoKeyStamper`) for Flow 36: stamps `record["_mongo_id"] = fc_id + "|" + sku_id + "|" + snapshot_date`.
- [ ] **STEP-24** — Implement `transformer_100` (`FCIDValidator`) for Flow 37: `fc_id == ""` → `nil` (silent drop). Otherwise passthrough.
- [ ] **STEP-25** — Implement `transformer_101` (`MovementTypeValidator`) for Flow 37: `movement_type` not in `{PUTAWAY, PICK, RETURN, DAMAGE, ADJUSTMENT}` → error `"transformer_101: unknown movement_type <value>"`. Otherwise passthrough.
- [ ] **STEP-26** — Implement `transformer_102` (`MongoKeyStamper`) for Flow 37: stamps `record["_mongo_id"] = movement_id`.

### Phase 7 — Pipeline Control Plane

- [ ] **STEP-27** — Implement `checkpoint_15` (Flow 36): after each successfully written batch, reads `_file_part`/`_file_row` off the last record in the batch (re-attached from `record.Meta` onto `record.Data` by flow_36's bridge — `CheckpointProps.Records` is `[]map[string]any`, so `Meta` never reaches it on its own) and upserts `fc_snapshot_progress` with `directory` (read from `param.State.GetReplicaProps()`), that `(last_part, last_row)`, `status='in_progress'`. Add an exported `MarkComplete(directory, auxMap)` that the flow's bridge calls once the source channel closes with no failure and no terminate-rule stop, setting `status='complete'`.
- [ ] **STEP-28** — Implement `checkpoint_16` (Flow 37): same mechanics against the single `fc_movement_progress` row — no `status` column and no `MarkComplete`, since this flow never "completes."
- [ ] **STEP-29** — Implement `backlog_15` (Flow 36): on any transformer or MongoDB write error, insert into `fc_snapshot_backlog` with `fc_id`, `sku_id`, `file_part`/`file_row` (read the same way as STEP-27), `failure_stage`, `error_message`, `record_payload` (JSONB of `record.Data`), `pipeline_run_id`.
- [ ] **STEP-30** — Implement `backlog_16` (Flow 37): same shape into `fc_movement_backlog` with `movement_id` in place of `sku_id`.
- [ ] **STEP-31** — Implement `terminate_15`/`terminate_16`: register **only** the built-in `MaxPipelineTime` field on `models.TerminateRuleTune` (30 min for Flow 36, 15 min for Flow 37) — no `UserDefinedCheckFunc`. This is a deliberate departure from Cases 4–5's terminate rules, which always poll connector-side "exhausted" state via a custom check func: `ReadByFullScan` here is a bounded, one-shot scan with no live connection to keep alive, so its channel just closes on its own once every listed file/row is delivered, and the engine's built-in `MaxPipelineTime` check (in `contexts.CheckTerminationConditions`) is purely a safety net for a wedged run, not the thing that ends a normal one.
...
- [ ] **STEP-38** — Start Flow 36 (pid=36). Verify it processes both Parquet parts and its source channel closes on its own — no terminate-rule stop. Confirm `fc_snapshot_progress.status = 'complete'` and `inventory_snapshots` has ~1,425 documents (1,500 total minus ~37 Fault A silent drops at 5% split evenly across two fault types, ~37 each).
- [ ] **STEP-39** — Start Flow 37 (pid=37). Verify it processes all 4 Avro parts, then its channel closes on its own once they're exhausted (this run is finished, not the directory). Confirm `stock_movements` has ~1,900 documents (2,000 total minus ~100 Fault A silent drops).
- [ ] **STEP-40** — Verify fault handling: confirm `fc_snapshot_backlog` has ~37 rows with `failure_stage=transform` and `transformer_98` error text; confirm `fc_movement_backlog` has ~50 rows with `transformer_101` unknown-movement_type errors.
- [ ] **STEP-41** — Test checkpoint/resume for Flow 36: kill it mid-scan. Note `(last_part, last_row)` in `fc_snapshot_progress`. Restart. Confirm it resumes from that exact position — `inventory_snapshots` document count does not change for rows already upserted (idempotent by `_id`).
...
- [ ] **STEP-44** — Full pipeline run: `make reset` → `make setup` → `make seed-snapshot` → `make seed-movement` → start Flow 36 → start Flow 37 → monitor via `make watch` until both channels close on their own. Verify final MongoDB counts and both backlog tables match the expected fault-rate math from STEP-38/39/40.
...
| Pipeline orchestrators | orchestrator_21 (Flow 36), orchestrator_22 (Flow 37) |
| Source 1 | Parquet directory-of-parts (connector_62 / iso_entity_152), Spark/Hive naming (`part-00000.parquet`), one fresh `dt=<date>` sub-directory per day under a stable configured `Directory` |
| Source 2 | Avro (OCF) directory-of-parts (connector_64 / iso_entity_154), same naming, single flat directory that never rotates — `Directory` and the on-disk path are the same |
...
| Position metadata | `record.Meta[MetaFilePart]`/`record.Meta[MetaFileRow]` — kept off `record.Data` by the framework; each flow's bridge re-attaches them onto `record.Data["_file_part"]`/`["_file_row"]` solely so `CheckpointProps`/`BacklogProps` (both `[]map[string]any`) can see them |
| Destination | MongoDB 7, database `fc_inventory`, collections `inventory_snapshots` (connector_63 / iso_entity_153) and `stock_movements` (connector_65 / iso_entity_155) — two separate connectors, since `DBMongoConfig.Collection` is fixed at connect time |
...
| Flow 36 transformer chain | `transformer_97` (SKUValidator) → `transformer_98` (QuantityGuard) → `transformer_99` (MongoKeyStamper) |
| Flow 37 transformer chain | `transformer_100` (FCIDValidator) → `transformer_101` (MovementTypeValidator) → `transformer_102` (MongoKeyStamper) |
| Checkpoints / Backlogs / Terminates | checkpoint_15/16, backlog_15/16, terminate_15/16 |
| Destination writes | destinationwrite_13 (Flow 36), destinationwrite_14 (Flow 37) |
...
| Termination | Both flows just let `ReadByFullScan`'s channel close on its own once their `GenerateScan`-bounded `Files` list is exhausted; `terminate_15`/`terminate_16` register only a `MaxPipelineTime` safety net (30 min / 15 min), no custom exhausted-polling check func. Flow 36 additionally marks `fc_snapshot_progress.status='complete'` on a clean finish; Flow 37 has no such terminal state — it's simply re-invoked periodically over the same growing directory. |
