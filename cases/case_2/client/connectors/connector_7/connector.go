package client_connector_7

// Zomato Platform Order Intelligence — connector_7: Hyperpure hot source (STEP-14)
//
// Redis stream source connector for the hyperpure brand hot flow.
// Reads WAL change events from "hyperpure:orders:stream" via XREADGROUP.
// Implements ReadBatch / AckBatch directly.
//
// iso_entities owned by this connector:
//   iso_entity_25 — orders
//   iso_entity_26 — order_items
//   iso_entity_27 — order_status_events
//   iso_entity_28 — delivery_assignments
...
func ReadBatch(ctx context.Context, redisClient models.IRedisClient, props *models.ConnectorProps, entityTable string) ([]map[string]any, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "NOGROUP") {
			return nil, fmt.Errorf("connector_7(%s): consumer group %q not found — ensure it is created before starting the hot flow", Brand, consumerGroup)
		}
		if isTimeout(err) {
			return nil, nil
		}
		return nil, fmt.Errorf("connector_7(%s): XREADGROUP: %w", Brand, err)
	}
