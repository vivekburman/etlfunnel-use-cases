package client_connector_5

// Zomato Platform Order Intelligence — connector_5: Zomato Food hot source (STEP-14)
//
// Redis stream source connector for the zomato_food brand hot flow.
// Reads WAL change events from "zomato:orders:stream" via XREADGROUP.
// Implements ReadBatch / AckBatch directly.
//
// iso_entities owned by this connector:
//   iso_entity_17 — orders
//   iso_entity_18 — order_items
//   iso_entity_19 — order_status_events
//   iso_entity_20 — delivery_assignments
...
// ReadBatch reads from the brand stream and filters to the requested entity table.
func ReadBatch(ctx context.Context, redisClient models.IRedisClient, props *models.ConnectorProps, entityTable string) ([]map[string]any, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("connector_5(%s): XREADGROUP: %w", Brand, err)
	}
...
// AckBatch acknowledges stream IDs after confirmed ES write.
