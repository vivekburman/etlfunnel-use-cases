package client_connector_6

// Zomato Platform Order Intelligence — connector_6: Blinkit hot source (STEP-14)
//
// Redis stream source connector for the blinkit brand hot flow.
// Reads WAL change events from "blinkit:orders:stream" via XREADGROUP.
// Implements ReadBatch / AckBatch directly.
//
// iso_entities owned by this connector:
//   iso_entity_21 — orders
//   iso_entity_22 — order_items
//   iso_entity_23 — order_status_events
//   iso_entity_24 — delivery_assignments
...
func ReadBatch(ctx context.Context, redisClient models.IRedisClient, props *models.ConnectorProps, entityTable string) ([]map[string]any, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "NOGROUP") {
			return nil, fmt.Errorf("connector_6(%s): consumer group %q not found — ensure it is created before starting the hot flow", Brand, consumerGroup)
		}
		if isTimeout(err) {
			return nil, nil
		}
		return nil, fmt.Errorf("connector_6(%s): XREADGROUP: %w", Brand, err)
	}
